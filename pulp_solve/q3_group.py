...
def CSR_fair_schedule(week_requires: List[List[int]], n_split: int = -1, shifts: List[List[int]] = SHIFTS,  minimum_day_off: int = MINIMUM_DAY_OFF) -> Tuple[int, List[int], List[List[int]], float]:
    # number of csr required in week
...
if __name__ == '__main__':
    num_csr, num_csr_each_day, week_schedule, _, _ = CSR_fair_schedule(REQUIRES)
