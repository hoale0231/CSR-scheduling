...
def check_requires_constraint_one_day(day_schedule: List[int], day_requires: List[int], shifts: List[List[int]]) -> List[List[int]]:
    """
...
def check_requires_constraint_all_day(week_schedule: List[List[int]], week_requires: List[List[int]], shifts: List[List[int]]) -> bool:
    """
    ...
    for idx, (day_result, day_requires) in enumerate(zip(week_schedule, week_requires)):
        try:
            check_requires_constraint_one_day(day_result, day_requires, shifts)
...
def check_maximum_onboard_day_constraint(week_schedule: List[List[int]], minimum_day_off: int = MINIMUM_DAY_OFF):
...
def check_fair_scheduling(week_schedule: List[List[int]], shifts: List[List[int]]):
